...
export const createUser = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    await sendEmail({
      email: saved.email,
      subject: `Welcome to Hotel Pearl - Your Account Has Been Created`,
  ...
  res.status(201).json({
    ...
    data: {
      ...
      providedPassword: password, // Include provided password in response
    },
  });
});
...
// ==============================================
// HOTEL BOOKING SYSTEM - DASHBOARD STATS
// ==============================================

// @desc    Get hotel booking dashboard overview
// @route   GET /api/v1/admin/dashboard/hotel-overview
// @access  Admin, Staff
export const getHotelDashboardOverview = catchAsyncErrors(async (req, res, next) => {
  ...
  res.json({
    success: true,
    message: "Hotel dashboard overview fetched successfully",
    data: {
      totalProperties,
      totalBookings,
      todayBookings,
...
// @route   GET /api/v1/admin/dashboard/booking-stats
// @access  Admin, Staff
export const getBookingStatsByDateRange = catchAsyncErrors(async (req, res, next) => {
  const { startDate, endDate } = req.query;
  ...
  while (currentDate <= end) {
    const dayData = dailyBookings.find(
      (d) =>
        d._id.year === currentDate.getFullYear() &&
        d._id.month === currentDate.getMonth() + 1 &&
        d._id.day === currentDate.getDate()
    ...
    result.push({
      ...
      bookings: dayData ? dayData.count : 0,
...
// @desc    Get property-wise booking statistics
// @route   GET /api/v1/admin/dashboard/property-stats
// @access  Admin, Staff
export const getPropertyWiseStats = catchAsyncErrors(async (req, res, next) => {
  ...
  res.json({
    success: true,
    message: "Property-wise statistics fetched successfully",
    data: propertyStats,
  });
});

// @desc    Get upcoming check-ins and check-outs
// @route   GET /api/v1/admin/dashboard/upcoming-activity
// @access  Admin, Staff
export const getUpcomingActivity = catchAsyncErrors(async (req, res, next) => {
  ...
  res.json({
    success: true,
    message: "Upcoming activity fetched successfully",
    data: {
      upcomingCheckIns,
      upcomingCheckOuts,
      dateRange: {
