...
export const stripeWebhook = async (req, res, next) => {
  ...
  if (!stripe) {
    console.warn("⚠️ Stripe webhook received but Stripe is not initialized");
    return res.status(503).json({ 
      error: "Stripe payment service is not available" 
    });
  }
  ...
  try {
    switch (event.type) {
      case "payment_intent.succeeded": {
        ...
        // ✅ Update booking if we have booking reference
        const bookingReference = metadata.booking_reference;
        if (bookingReference) {
          const Booking = (await import("../models/bookingModel.js")).default;
          const booking = await Booking.findOne({ bookingReference }).populate("property_id", "owner_id");
          if (booking) {
            ...
            if (req.app?.get) {
              ...
              if (io) {
                ...
                io.to(`user:${booking.property_id.owner_id}`).emit("booking.payment.success", {
